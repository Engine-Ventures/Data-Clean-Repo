...
from evpipeline import connect, lookup  # noqa: E402
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _lookup(self, query: str) -> None:
        ...
        if not _lookup_enabled:
            self._json(200, {**lookup.Suggestion(
                note="lookup disabled with --no-lookup"
            ).as_dict(), "enabled": False})
            return
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("  lookup " + ("wikidata.org (company names leave this machine)"
                        if _lookup_enabled else "off (--no-lookup)"))
    try:
        httpd.serve_forever()
